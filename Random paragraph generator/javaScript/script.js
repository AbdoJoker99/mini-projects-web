...
function generate(){
    if (items.value ==0){
        alert("the vlaue  cannot be zero")
    }else if(items.value>paragraphs.length){
        alert("the value cannot be greater than the number of paragraphs");
        const randomIndex =Math.floor(Math.random()*paragraphs.length);
        dataContainer.innerHTML=`${paragraphs[randomIndex]}`;

    }else{
       const suffelParagraphs=paragraphs;

       shuffle(paragraphs);
       const randomParagraphs = paragraphs.slice(0, items.value);
       const randomParagraphsHtml = randomParagraphs.map((paragraph) =>`<p>${paragraph}</p>`).join("");

       dataContainer.innerHTML=randomParagraphsHtml;
